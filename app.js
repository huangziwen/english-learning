...
(function () {
  ...
  function updatePlayButtons() {
    ...
    // Sentence analysis play buttons
    document.querySelectorAll('.sa-play-btn').forEach(function (btn) {
      var target = 'sa_' + btn.dataset.sentence.substring(0, 10);
      if (isPlaying && speakingTarget === target) {
        btn.classList.add('playing');
        btn.innerHTML = '&#10074;&#10074;';
      } else {
        btn.classList.remove('playing');
    ...
    // Sentence analysis word play buttons
    document.querySelectorAll('.sa-word-play').forEach(function (btn) {
      var target = 'saword_' + btn.dataset.wordAudio;
      if (isPlaying && speakingTarget === target) {
        btn.classList.add('playing');
        btn.innerHTML = '&#10074;&#10074;';
      } else {
  ...
  function renderHome() {
    ...
    html += '<p>六大知识分类 + BBC 中长篇阅读，从零基础到专业采购，每课配有纯正音频朗读与逐句逐词中文解析</p>';
    html += '</div>';
  ...
  function renderLesson() {
    ...
    // Sentence-by-Sentence Word Analysis (for BBC, Business, Procurement, ProcurementCategories)
    var sa = (typeof SENTENCE_ANALYSIS !== 'undefined') ? SENTENCE_ANALYSIS[lesson.id] : null;
    if (sa) {
      html += '<div class="content-section sentence-analysis-section">';
      html += '<div class="section-title"><span class="section-icon">&#128269;</span> 逐句逐词解析</div>';
      html += '<div class="sa-intro">点击任意单词可听发音 · 共 ' + sa.length + ' 句</div>';
      sa.forEach(function (s, sIdx) {
        html += '<div class="sa-sentence-block">';
        html += '<div class="sa-sentence-header">';
        html += '<span class="sa-sentence-num">' + (sIdx + 1) + '</span>';
        html += '<button class="sa-play-btn" data-sentence="' + escapeHtml(s.sentence) + '">&#9654;</button>';
        html += '</div>';
        html += '<div class="sa-sentence-en">' + escapeHtml(s.sentence) + '</div>';
        html += '<div class="sa-sentence-zh">' + escapeHtml(s.translation) + '</div>';
        html += '<div class="sa-words-grid">';
        s.words.forEach(function (w) {
          html += '<div class="sa-word-card" data-word="' + escapeHtml(w.word) + '">';
          html += '<div class="sa-word-top">';
          html += '<span class="sa-word-text">' + escapeHtml(w.word) + '</span>';
          html += '<button class="sa-word-play" data-word-audio="' + escapeHtml(w.word) + '">&#9654;</button>';
          html += '</div>';
          if (w.phonetic) {
            html += '<div class="sa-word-phonetic">' + escapeHtml(w.phonetic) + '</div>';
          }
          html += '<div class="sa-word-meaning">' + escapeHtml(w.meaning) + '</div>';
          html += '</div>';
        });
        html += '</div>';
        html += '</div>';
    ...
    // Sentence analysis: sentence play buttons
    document.querySelectorAll('.sa-play-btn').forEach(function (btn) {
      btn.addEventListener('click', function (e) {
        e.stopPropagation();
        var sentence = btn.dataset.sentence;
        var target = 'sa_' + sentence.substring(0, 10);
        if (isPlaying && speakingTarget === target) {
          stopSpeaking();
        } else {
          speak(sentence, target);
        }
      });
    });
    ...
    document.querySelectorAll('.sa-word-play').forEach(function (btn) {
      btn.addEventListener('click', function (e) {
        e.stopPropagation();
        var word = btn.dataset.wordAudio;
        var target = 'saword_' + word;
        if (isPlaying && speakingTarget === target) {
          stopSpeaking();
        } else {
          speak(word, target);
